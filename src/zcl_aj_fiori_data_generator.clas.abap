CLASS zcl_aj_fiori_data_generator DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
CLASS ZCL_AJ_FIORI_DATA_GENERATOR IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.

    DATA lt_travel TYPE STANDARD TABLE OF zaj_fiori_travel WITH EMPTY KEY.
    DATA lt_book   TYPE STANDARD TABLE OF zaj_fiori_book   WITH EMPTY KEY.
    DATA lt_suppl  TYPE STANDARD TABLE OF zaj_fiori_suppl  WITH EMPTY KEY.
    ...
    TRY.
        ...
        "------------------------------------------------------------
        DELETE FROM zaj_fiori_suppl.
        lv_suppl_deleted = sy-dbcnt.

        DELETE FROM zaj_fiori_book.
        lv_book_deleted = sy-dbcnt.

        DELETE FROM zaj_fiori_travel.
        ...
        IF lt_travel IS NOT INITIAL.
          INSERT zaj_fiori_travel FROM TABLE @lt_travel.
          lv_travel_insert = sy-dbcnt.
        ENDIF.

        IF lt_book IS NOT INITIAL.
          INSERT zaj_fiori_book FROM TABLE @lt_book.
          lv_book_insert = sy-dbcnt.
        ENDIF.

        IF lt_suppl IS NOT INITIAL.
          INSERT zaj_fiori_suppl FROM TABLE @lt_suppl.
        ...
        out->write( |ZAJ_FIORI_TRAVEL  : { lv_travel_deleted } records| ).
        out->write( |ZAJ_FIORI_BOOK    : { lv_book_deleted } records| ).
        out->write( |ZAJ_FIORI_SUPPL   : { lv_suppl_deleted } records| ).
        out->write( | | ).

        out->write( |Custom data inserted:| ).
        out->write( |ZAJ_FIORI_TRAVEL  : { lv_travel_insert } records| ).
        out->write( |ZAJ_FIORI_BOOK    : { lv_book_insert } records| ).
        out->write( |ZAJ_FIORI_SUPPL   : { lv_suppl_insert } records| ).
